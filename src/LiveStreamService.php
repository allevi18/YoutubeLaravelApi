...
//          $this->googleLiveBroadcastStatus->setSelfDeclaredMadeForKids(true);
            ...
            if ($video instanceof Video) {
                ...
                if (!is_null($language)) {
                    ...
                    $video->offsetSet('defaultAudioLanguage', $temp);
                    $video->offsetSet('defaultLanguage', $temp);
                }
            }
    ...
    public function updateBroadcast($token, $data, $youtubeEventId)
    {
        try {
            ...
            if ($video instanceof Video) {
                ...
                $video->offsetSet('title', $title);
                $video->offsetSet('description', $description);
                $video->offsetSet('scheduledStartTime', $startdtIso);
